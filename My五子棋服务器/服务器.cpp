...
void dosign(int clinum) {//处理用户注册
	...
	if (rownum == 0) {//用户名不存在
		...
		sprintf(sendbuffer, "_sign sign_success ");
		...
		//memset(sendbuffer,0,sizeof(sendbuffer));
		sprintf(sendbuffer, "_sign username_exist ");
		send(clifd[clinum], sendbuffer, strlen(sendbuffer), 0);
	}
...
void dolog(int clinum) {//处理用户登陆
	...
	if (rownum == 0) {//不存在用户名
		//memset(sendbuffer,0,sizeof(sendbuffer));
		sprintf(sendbuffer, "_log username_wrong ");
		send(clifd[clinum], sendbuffer, strlen(sendbuffer), 0);
	}
	else {//用户名存在
		//密码是否正确
		...
		if (strcmp(row[1], password) != 0) {//密码错误
			//memset(sendbuffer,0,sizeof(sendbuffer));
			sprintf(sendbuffer, "_log password_wrong ");
			send(clifd[clinum], sendbuffer, strlen(sendbuffer), 0);
			mysql_free_result(res);
			return;
		}
			...
			if (strcmp(username, cliusername[1 - clinum]) == 0) {
				sprintf(sendbuffer, "_log user_loged ");
				send(clifd[clinum], sendbuffer, strlen(sendbuffer), 0);
			...
			//memset(sendbuffer,0,sizeof(sendbuffer));
			sprintf(sendbuffer, "_log password_right ");
			send(clifd[clinum], sendbuffer, strlen(sendbuffer), 0);
...
void doregretrequest(int clinum) {//处理悔棋请求
	sprintf(sendbuffer, "_other_want_to_regret ");
	send(clifd[1 - clinum], sendbuffer, strlen(sendbuffer), 0);
	printf("sendbuffer : %s\n", sendbuffer);
	printf("向用户%s发送悔棋请求成功\n", cliusername[1 - clinum]);
}

bool doregretrespond(int clinum) {//处理悔棋响应
	int result = -1;
	sscanf(recvbuffer + recvbufferidx, "%d", &result);
	recvbufferidx += strlen(recvbuffer) + 1;
	if (result != 0 && result != 1) {
		printf("响应result获取错误\n");
		return false;
	}
	if (result == 0) {
		sprintf(sendbuffer, "_regret_is_fail ");
		send(clifd[1 - clinum], sendbuffer, strlen(sendbuffer), 0);
		printf("sendbuffer : %s\n", sendbuffer);
		printf("向用户%s发送悔棋响应成功\n", cliusername[1 - clinum]);
		return false;
	}
	if (result == 1) {
		sprintf(sendbuffer, "_regret_is_ok %d %d ", ptail->x, ptail->y);
		send(clifd[1 - clinum], sendbuffer, strlen(sendbuffer), 0);
		printf("sendbuffer : %s\n", sendbuffer);
		printf("向用户%s发送悔棋响应成功\n", cliusername[1 - clinum]);
		sprintf(sendbuffer, "_regret_chess %d %d ", ptail->x, ptail->y);
		send(clifd[clinum], sendbuffer, strlen(sendbuffer), 0);
		printf("sendbuffer : %s\n", sendbuffer);
		printf("向用户%s发送悔棋响应成功\n", cliusername[clinum]);

		chessboard[ptail->x][ptail->y] = 0;
...
bool doreturnmenu(int clinum) {//游戏开始返回true;否则，返回false;
	...
	if (cliuserlog[1 - clinum]) {
		if (!cliuserready[0] || !cliuserready[1]) {//游戏未开始
			sprintf(sendbuffer, "_other_return_to_menu _no_game_start ");
			send(clifd[1 - clinum], sendbuffer, strlen(sendbuffer), 0);
			printf("sendbuffer");
			...
			sprintf(sendbuffer, "_other_return_to_menu _game_start ");
			send(clifd[1 - clinum], sendbuffer, strlen(sendbuffer), 0);
			printf("sendbuffer");
			printf("向用户%s发送用户%s已返回菜单的消息成功\n", cliusername[1 - clinum], cliusername[clinum]);
			return true;
...
void domessage(int clinum) {//clinum是向服务器发送请求的客户端；flag标志：是否向对方服务器发送请求
	...
	while (recvbufferidx < strlen(recvbuffer)) {
		...
		if (strcmp(waybuffer, "_I_want_to_regret") == 0) {//悔棋请求
			doregretrequest(clinum);
		}

		if (strcmp(waybuffer, "_answer_regret_result") == 0) {//悔棋请求
			if (doregretrespond(clinum)) {
				printf("chessexitnum = %d\n", chessexitnum);
				doround(clinum);
			}
		}
